...
import { syncPillarBlogSections } from "./lib/pillar-injector.js";
...
const SUITE_AI_DIR = join(REPO_ROOT, "kai-website", "src", "suite-ai");
...
async function main() {
  ...
  // Aggiorna la sezione "Dal blog" nelle pagine pillar (link pillar → articolo,
  // richiesto dalle regole SEO: link bidirezionale pillar ↔ cluster).
  const pillarFilesModified = syncPillarBlogSections(BLOG_DIR, SUITE_AI_DIR);
  console.log(`[pillar] sezioni "Dal blog" aggiornate: ${pillarFilesModified.join(", ") || "nessuna"}`);
  ...
  const imageRels = images
    ...
    : [];
  const scheduleRel = `tools/blog-bot/schedule.json`;

  // Marca la riga PRIMA del commit: markBlogPublished riscrive schedule.json,
  // e il commit qui sotto deve includere quella modifica. Se marcassimo dopo
  // il commit, la modifica non arriverebbe su origin e il bot ripescherebbe
  // la stessa riga al run successivo (duplicati).
  ...
  // Git commit + push (include schedule.json già marcato)
  ...
  const pillarRels = pillarFilesModified.map((f) => `kai-website/src/suite-ai/${f}`);
  commitAndPush([articleRel, indexRel, sitemapRel, scheduleRel, ...imageRels, ...pillarRels], msg, REPO_ROOT);
