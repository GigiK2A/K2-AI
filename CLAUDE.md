...
| LLM API | **OpenAI + modello locale** (Anthropic spento dal 19 ago 2026, OK Luca) | Scelta per USO, non per potenza: agenti che fanno il loro lavoro (heartbeat, proposte, azioni) → **locale** `gpt-oss` su Ollama, costa zero e può aspettare; agenti IN CHAT e istruzioni scritte → **OpenAI** `gpt-4o`/`gpt-4o-mini` (Chat Completions via urllib, nessun SDK). Dietro ognuno la catena di riserve, che entra solo se il primario è inutilizzabile (401/429/5xx/timeout). Ricerca clienti e competitor: OpenAI Responses API col tool `web_search` — senza ricerca web il modello si inventa le aziende. **Anthropic è fuori dal giro**: il codice resta, rientra solo con `AIOS_USA_ANTHROPIC=1` e una chiave valida. Primario configurabile con `AIOS_LLM_BACKEND` (`openai`\|`local`). |
