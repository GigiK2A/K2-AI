...
| LLM API | **OpenAI + modello locale** (Anthropic spento dal 19 ago 2026, OK Luca) | **OpenAI è il primario per tutto** (`AIOS_LLM_BACKEND=openai`, 20 ago 2026): `gpt-4o` per il giudizio, `gpt-4o-mini` per i passi economici. Il **locale** `gpt-oss` su Ollama resta come riserva gratuita se OpenAI è giù. Misurato sul ledger di un mese, tutto il traffico dei reparti su OpenAI costa ~2 €/mese: a queste volumetrie il locale non serviva a risparmiare (Chat Completions via urllib, nessun SDK). Dietro ognuno la catena di riserve, che entra solo se il primario è inutilizzabile (401/429/5xx/timeout). Ricerca clienti e competitor: OpenAI Responses API col tool `web_search` — senza ricerca web il modello si inventa le aziende. **Anthropic è fuori dal giro**: il codice resta, rientra solo con `AIOS_USA_ANTHROPIC=1` e una chiave valida. Primario configurabile con `AIOS_LLM_BACKEND` (`openai`\|`local`). |
...
4. Integrare SaaS a pagamento non in elenco (**OpenAI è approvato e ora è il fornitore principale**: web search K-BOT giu 2026, immagini lug 2026, chat e ricerca clienti del board ago 2026 — vedi §3)
