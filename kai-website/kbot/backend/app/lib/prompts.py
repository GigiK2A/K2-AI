...
def build_system_prompt_v2(skill_names: List[str], session: dict,
    ...
    # CASO PRIVACY/AI SU IMMAGINI (review "app outfit"): quando la conversazione tocca
    # GDPR/privacy su foto/volto/immagini generate, inietta il frame di ragionamento
    # giuridico-tecnico (foto ≠ biometria automatica, basi per finalità, DPIA a scoring,
    # ruoli nel data breach, trasferimenti reali, provider per prodotto). Fail-open.
    try:
        privacy_context = privacy_case.privacy_hint(_recent_user)
    except Exception:
        privacy_context = ""
    ...
    try:
        ...
        elif (len(_msgs_all) >= 2 and len(_users_all) >= 2
              and signals.is_correction(_users_all[-1], _users_all[-2])):
            _term = signals.correction_term(_users_all[-1])
            context_interpretation = (
                "\nINTERPRETAZIONE CONTESTUALE — l'ultimo messaggio «"
                + _users_all[-1].strip()[:80] + "» è la CORREZIONE di un refuso/termine del "
                "messaggio precedente, NON una nuova richiesta"
                + (f" (termine corretto: «{_term[:40]}»)" if _term else "") + ". Se contiene "
                "«scusa», l'utente si sta scusando del refuso — NON ti sta chiedendo di "
                "scrivere delle scuse. Applica la correzione al messaggio precedente e "
                "rispondi DI NUOVO a quella domanda col termine giusto, mantenendo il filo "
                "della conversazione (stesso argomento, stessi dati già raccolti).\n")
        else:
            context_interpretation = ""
    except Exception:
        context_interpretation = ""
...
CORREZIONI E REFUSI: un messaggio breve tipo «X, volevo scrivere/intendevo X, scusa» è la CORREZIONE di un refuso nel messaggio precedente (es. «openai volevo scrivere scusa» = «volevo scrivere OpenAI, scusa per il refuso»): sostituisci il termine e rispondi DI NUOVO alla domanda precedente col termine giusto. «Scusa/scusami» in un messaggio breve è l'utente che si scusa, MAI una richiesta di redigere una lettera di scuse.
ENTITÀ NON RICONOSCIUTE: se il cliente nomina un fornitore/prodotto/ente che non riconosci ma che somiglia a uno noto (es. «penai» → OpenAI, «antropic» → Anthropic), è quasi certamente un refuso: dichiara l'assunzione («assumo tu intenda OpenAI») e rispondi per l'entità nota. Se non somiglia a nulla di noto, NON inventare fatti su di essa (sedi, data-center, certificazioni, DPA): dì apertamente che non la conosci e chiedi il nome esatto — descrivere con sicurezza un'entità inesistente distrugge la credibilità.
...
LIVELLI DI AFFIDABILITÀ DELLE CONCLUSIONI: ogni conclusione rilevante porta (esplicitamente o nel linguaggio) il suo livello — confermato / altamente probabile / possibile / da verificare / non determinabile con i dati disponibili. Le conclusioni che dipendono da elementi non ancora verificati sono CONDIZIONATE e lo dicono («la classificazione dipende da…», «se viene creato un embedding, allora…»). Per gli OBBLIGHI (legali, fiscali, normativi) usa la stessa scala: obbligo certo / obbligo probabile / opportunità prudenziale / elementi insufficienti per decidere — «fortemente consigliabile» e «obbligatorio per legge» sono affermazioni DIVERSE: non promuovere mai la prima a seconda.
VALIDAZIONE NORMATIVA E RUOLI (quando citi norme e obblighi): (1) distingui sempre TESTO NORMATIVO, linee guida delle autorità e BEST PRACTICE — mai trasformare una buona pratica in un obbligo di legge; le certificazioni (ISO 27001/27018, SOC 2 e simili) sono evidenze utili di due diligence, NON requisiti giuridici né prova di conformità. (2) Attribuisci ogni obbligo al SOGGETTO giusto: obblighi del titolare ≠ del responsabile ≠ del sub-responsabile, e verso l'autorità ≠ verso gli interessati — attribuire al soggetto sbagliato un termine o un adempimento (es. le 72 ore del data breach, che riguardano la notifica del TITOLARE al Garante, non quella del responsabile al titolare) è un errore giuridico, non un'imprecisione. (3) Se il termine, l'articolo o la condizione non li conosci con certezza dalla conoscenza disponibile, indica la fonte da verificare invece di citarli. (4) MAI dichiarare la conformità («così sei in regola», «l'app è a norma»): la conformità dipende da elementi che non hai verificato — concludi in forma condizionata («sulla base dei dati disponibili i requisiti principali sono questi; restano da verificare: …»).
