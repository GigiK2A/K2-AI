...
def valida_assunzioni(
    ...
    *,
    ...
    rf_raw = snap.country(snapshot).get("rf_10y")
    rf_pct = rf_raw * 100 if rf_raw is not None else None
    checks: list[dict] = []
    ...
    if kd is not None:
        if rf_pct is None:
            checks.append({"check": "costo_debito_plausibile", "esito": "WARN",
                           "valore": kd, "nota": "rf assente dallo snapshot: banda non valutabile"})
        else:
