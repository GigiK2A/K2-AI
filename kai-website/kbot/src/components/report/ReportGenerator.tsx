...
export function ReportGenerator({
  ...
  async function generate(attempt = 0, servizioOverride?: string) {
    setBusy(true);
    setErr(null);
    setPdfUrl(null);
    setCheckout(null);
    ...
    try {
      const token = getAuthToken ? await getAuthToken() : null;
      const res = await autoGenerateDeliverable(sessionId, token, servizioOverride);
      if (res.kind === "reroute" && !servizioOverride) {
        // OUTPUT ALIGNMENT CHECKER: il backend propone il documento giusto dal
        // catalogo → rigeneriamo DA SOLI con quello (mai un vicolo cieco). Una
        // sola volta: col servizio esplicito il backend non rigiudica (no loop).
        setLabel(res.suggested_label ?? res.servizio_suggerito);
        setStatus("routed");
        return await generate(attempt, res.servizio_suggerito);
      }
      if (res.kind === "reroute") {
        // reroute su un servizio già esplicito: non dovrebbe accadere — messaggio onesto.
        setErr(res.message);
        setBusy(false);
        return;
      }
      ...
      if (final.status === "rendered") {
        ...
        setStatus("running");
        return await generate(attempt + 1, servizioOverride);
