...
def _classify(regola_testo: str) -> str:
    ...
    if "json" in t and ("valida" in t or "contratto" in t):
        return "json_output"
    # coverage 1:1 voce->slot (R13). Prima dei rami generici: contiene "contratto"
    # ma non "json", quindi non collide con json_output.
    if "slot" in t and ("copertura" in t or "output-schema" in t or "contratto" in t or "1:1" in t):
        return "coverage_slot"
    # voci extra/duplicate (R14). DEVE precedere il ramo exec_summary, altrimenti
    # "executive summary doppio" verrebbe classificato come exec_summary.
    if "extra" in t or "duplicat" in t or "doppio" in t:
        return "voci_ordine"
...
def _check(kind: str, blueprint: dict, instance: dict) -> list[str]:
    ...
    if kind == "voci_ordine":
        ...
        # voci EXTRA non previste dal blueprint (#21b-res): catturano la struttura
        # libera (sezioni inventate dall'agente). Gli id-sentinella "__extra__:slug"
        # prodotti dall'adapter sotto validazione_strict finiscono qui.
        extra = [i for i in present if i and i not in ordered_bp]
        if extra:
            pretty = [str(i).split(":", 1)[1] if str(i).startswith("__extra__:") else i for i in extra]
            msgs.append(f"voci extra non previste dal blueprint: {pretty}")
        # voci DUPLICATE (es. executive summary doppio: hero + capitolo numerato)
        counts: dict = {}
        for i in present:
            if i in ordered_bp:
        ...
        if dup:
            msgs.append(f"voci duplicate: {dup}")
    ...
    elif kind == "coverage_slot":
        # copertura 1:1 voce->slot a livello blueprint (verifica pura, no FS):
        # ogni voce deve dichiarare uno `slot`. L'esistenza effettiva dello slot nel
        # file contratto_dati (output-schema) è verificata da server.check_coverage_impl
        # (richiede accesso al filesystem).
        for v in voci_bp:
            if not v.get("slot"):
                msgs.append(f"voce '{v.get('id')}' senza slot dati dichiarato (copertura voce->contratto_dati incompleta)")
...
def lint(blueprint: dict, instance: dict) -> dict:
    ...
    # dedup: regole diverse instradate allo stesso check producono lo stesso finding
    # (es. R06 e R14 -> voci_ordine). Tieni la prima occorrenza (con il suo id-regola).
    _seen: set = set()
    _dedup: list[dict] = []
    for f in findings:
        key = (f.get("tipo"), f.get("messaggio"))
        if key in _seen:
            continue
        _seen.add(key)
        _dedup.append(f)
