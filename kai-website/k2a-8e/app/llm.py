...
_QUALITA_TRASVERSALE = (
    ...
    "  • FATTI vs IPOTESI vs CONCLUSIONI (onestà diagnostica, PRIORITARIO): i FATTI sono SOLO ciò "
    "che il cliente ha confermato — non reinterpretarli. Una DEDUZIONE è un'IPOTESI, non un fatto: "
    "«le 4 assunzioni hanno ridotto la produttività» NON è un fatto → scrivi «le informazioni "
    "raccolte suggeriscono che la crescita dell'organico potrebbe non aver prodotto un incremento "
    "proporzionale dell'efficienza — ipotesi principale, da verificare». Ogni ipotesi con un livello "
    "di probabilità; conclusioni definitive SOLO se i dati bastano, altrimenti diagnosi PRELIMINARE "
    "dichiarata come tale.\n"
    "  • CITA LE PAROLE DEL CLIENTE: se dalla consulenza emerge una frase significativa, RIPORTALA "
    "testualmente e rendila centrale (es. «l'elemento che emerge con più forza non è il calo "
    "dell'utile, ma la percezione dell'imprenditore di una perdita di coordinamento interno "
    "nonostante l'elevato impegno del personale»). Il report deve sembrare scritto da chi ha "
    "ASCOLTATO davvero il cliente, non un template col nome cambiato.\n"
    "  • NIENTE ENTITÀ INVENTATE: non introdurre figure aziendali (HR, Direttore Operativo, PMO, "
    "Controller, CIO, IT, Team Leader) né strumenti (Power BI, Tableau, SAP, ERP, CRM) che il "
    "cliente NON ha mai citato. Rifletti la realtà dichiarata: una PMI di 8 persone non ha un PMO.\n"
    "  • AZIONI PROPORZIONATE ALLA DIMENSIONE: prima di proporre dashboard, KPI, software, business "
    "intelligence, onboarding o workflow complessi, valuta il livello organizzativo dell'azienda. "
    "Spesso una piccola PMI trae più beneficio da: chiarire i ruoli, eliminare le interruzioni, "
    "rivedere il flusso delle commesse, ridefinire le priorità, standardizzare le attività "
    "ripetitive. Proponi il livello d'intervento CORRETTO per QUELLA azienda.\n"
    "  • INCERTEZZA DICHIARATA: quando i dati mancano, dillo — «valutazione basata sulle "
    "informazioni raccolte in consulenza: diagnosi preliminare; alcune ipotesi potranno essere "
    "confermate, ridimensionate o escluse una volta disponibili i dati economici e operativi "
    "richiesti». Mai fingere precisione.\n"
